...
@IBDesignable open class RangeSlider: UIControl {
  ...
  open func updateTrackLayerFrameAndKnobPositions() {
    ...
    open func setNumDecimals(num value: Int){
        self.numDecimals = value
    }
    
    open func getLabelText(forValue value: Double) -> String {
        let numberFormatter = NumberFormatter()
        numberFormatter.numberStyle = NumberFormatter.Style.decimal
        numberFormatter.maximumFractionDigits = self.numDecimals
        guard var labelText = numberFormatter.string(from: NSNumber(value: value)) else { return "" }
        
        if(value < 1 && labelText.count == 3){
    ...
    open func getLowerLabel() -> Double {
        let numberFormatter = NumberFormatter()
        numberFormatter.numberStyle = NumberFormatter.Style.decimal
        numberFormatter.maximumFractionDigits = self.numDecimals
        let lowerString = numberFormatter.string(from: NSNumber(value: lowerValue))!
        let lowerStringFinal = lowerString.replacingOccurrences(of: ",", with: "", options: NSString.CompareOptions.literal, range: nil)
        let convertedDouble = (lowerStringFinal as NSString).doubleValue
        return convertedDouble
    }
    
    open func getUpperLabel() -> Double {
        let numberFormatter = NumberFormatter()
        numberFormatter.numberStyle = NumberFormatter.Style.decimal
        numberFormatter.maximumFractionDigits = self.numDecimals
        let upperString = numberFormatter.string(from: NSNumber(value: upperValue))!
        let upperStringFinal = upperString.replacingOccurrences(of: ",", with: "", options: NSString.CompareOptions.literal, range: nil)
        let convertedDouble = (upperStringFinal as NSString).doubleValue
        return convertedDouble
    }
